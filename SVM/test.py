...
LastEditor: JiangJi
LastEditTime: 2021-08-31 20:59:45
Discription: 
Environment: 
'''
import numpy as np
...
# model_linear = svm.SVC(kernel='linear', C=C).fit(data, label)
...
for i, gamma in enumerate([1, 5, 15, 35, 45, 55]):
    rbf_svc = svm.SVC(kernel='rbf', gamma=gamma, C = 0.0001).fit(data, label)
...
model_poly = svm.SVC(C=0.0001, kernel='poly', degree=10).fit(data, label) # 多项式核
