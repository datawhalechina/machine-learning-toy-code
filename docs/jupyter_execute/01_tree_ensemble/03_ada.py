...
# $$
# 
# 从概率角度而言，一个设计良好的分类问题损失函数应当保证模型在期望损失达到最小时的输出结果$k^*$是使得后验概率$P(S(\mathbf{y})=k\vert \mathbf{x}),k\in\{1,...,K\}$达到最大的类别\mathop{\arg\max}_k P(S(\mathbf{y})=k\vert \mathbf{x})，这个条件被称为贝叶斯最优决策条件。在本问题下，满足对称约束条件的损失函数期望损失$\mathbb{E}_{\mathbf{Y}\vert\mathbf{x}}L(\mathbf{Y},f)$达到最小时，由拉格朗日乘子法可解得模型输出为
# 
# $$
# \begin{aligned}
