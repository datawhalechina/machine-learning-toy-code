...
# 
# 条件三看上去比较复杂，但其意义是容易理解的，即$n$个事件拆分为$n+1$个事件时的不确定性增加了，并且增加的不确定性与拆分时的比例和拆分事件的概率有关。举例来说：将$\textbf{p}=[0.9,0.1]$分别拆分为$\textbf{p}_1=[0.45,0.45,0.1]$和$\textbf{p}_2=[0.9,0.05,0.05]$，那么显然$\textbf{p}_1$增加的不确定性远超过$\textbf{p}_2$；同时，将$\textbf{p}=[0.9,0.1]$分别拆分为$\textbf{p}_3=[0.8,0.1,0.1]$，那么显然$\textbf{p}_1$增加的不确定性也远超过$\textbf{p}_2$。
# 
